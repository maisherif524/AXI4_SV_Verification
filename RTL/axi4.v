module axi4 #(
    ...
    reg [ADDR_WIDTH-1:0] write_addr, read_addr, write_addr_cross, read_addr_cross;
    ...
    assign write_boundary_cross = ((write_addr_cross & 12'hFFF) + (write_burst_len << write_size)) > 12'hFFF;
    assign read_boundary_cross = ((read_addr_cross & 12'hFFF) + (read_burst_len << read_size)) > 12'hFFF;
    ...
    // FSM states
    reg [2:0] write_state;
    localparam W_IDLE = 3'd0, //000
               W_ADDR = 3'd1, //001
               W_DATA = 3'd2, //010
               W_RESP = 3'd3; //011

    reg [2:0] read_state;
    localparam R_IDLE = 3'd0,  //000
               R_ADDR = 3'd1,  //001
               R_DATA = 3'd2;  //010

    // Registered memory read data for timing
    // reg [DATA_WIDTH-1:0] mem_rdata_reg;      not used in this implementation
    ...
    always @(posedge ACLK or negedge ARESETn) begin
        if (!ARESETn) begin
            ...
            write_addr_cross <= {ADDR_WIDTH{1'b0}};
            read_addr_cross <= {ADDR_WIDTH{1'b0}};
